...
hprx() {
  if [ "$1" == "--help" ]; then
    ...
    echo "Usage: hprx <COMMAND>"
    echo COMMAND: start|stop|status
    return
  fi
  if [ "$1" == "start" ]; then
    echo Starting heisenberg proxy...
    ssh -o "ProxyCommand java -Xmx64m -cp 'C:\Users\cvila\Dev\Perso\cloud.vilanet.fr\ssl\out\production\ssl' org.antiloop.ssl.SSLTunnel -p 10.43.216.8:8080 - 37.187.116.136:443" -i "C:\Users\cvila\Documents\Nextcloud\Perso\Serveurs\Kimsufi\ns329449_private.key" -p 443 -l kriss -L 127.0.0.1:3128:127.0.0.1:3128 -S /tmp/sock-heisenberg -M -f -N 37.187.116.136
